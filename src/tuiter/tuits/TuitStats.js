...
const TuitStats=({post})=>{
  return(
      <div className="col-10 pt-3 d-flex justify-content-between text-muted">
        ...
        {post.liked &&<Link className="text-decoration-none text-muted"><FontAwesomeIcon style={{color:"red"}}
            icon={faHeart}/> {post.likes}</Link>}
        {!post.liked &&<Link className="text-decoration-none text-muted"><FontAwesomeIcon
           icon={faHeart}/> {post.likes}</Link>}
        <Link className="text-decoration-none text-muted"><FontAwesomeIcon
